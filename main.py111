...
if not TELEGRAM_TOKEN or not ASSEMBLYAI_KEY or not GEMINI_KEY:
    logging.warning("One or more API keys are missing. Please ensure TELEGRAM_TOKEN, ASSEMBLYAI_KEY, and GEMINI_KEY are set.")
...
if not FFMPEG_BINARY:
    ...
    for p in POSSIBLE_FFMPEG_PATHS:
        try:
            ...
            continue
if not FFMPEG_BINARY:
    logging.warning("ffmpeg not found. Please install ffmpeg or set the FFMPEG_BINARY variable.")
...
TTS_VOICE_DEFAULT = "en-US-JennyMultilingualNeural"
...
def extract_audio_to_wav(video_path, out_rate=16000):
    ...
    cmd = [ensure_ffmpeg(), "-y", "-i", video_path, "-vn", "-acodec", "pcm_s16le", "-ar", str(out_rate), "-ac", "1", tmp_name]
    try:
...
def build_segments_from_gentle(alignment_json, gap_ms=300, min_segment_words=1):
    ...
    if not segments and words:
        ...
        if aligned_words:
            ...
            segments.append({'text': full_text, 'start': 0, 'end': 1000})
...
def callback_query(call):
    ...
    try:
        if data.startswith("src|"):
            ...
            markup.add(*buttons)
            try:
                bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=f"Original language: {lang}. Select the dubbing language:", reply_markup=markup)
...
def build_segments_from_asr(transcript_json, gap_ms=400):
    ...
    if not words:
        ...
        if full_text:
            segments.append({'text': full_text, 'start': 0, 'end': audio_duration_ms or 1000})
