...
async def process_video(chat_id, data):
    ...
    try:
        ...
        if trans_resp.status_code not in (200, 201):
            if status_info:
                bot.edit_message_text(chat_id=status_info['chat_id'], message_id=status_info['message_id'], text="Processing: Failed to start transcription.")
            else:
        ...
        if retry_count >= max_retries:
            if status_info:
                bot.edit_message_text(chat_id=status_info['chat_id'], message_id=status_info['message_id'], text="Processing: Transcription timed out.")
            else:
